...
export function AdminCategoriesManager({ initialCategories }: { initialCategories: any[] }) {
  ...
  const openCreateModal = () => {
    ...
    setSlug("");
    setIcon("Activity");
    setSortOrder((categories.length + 1) * 1);
    setIsActive(true);
    const map: any = {};
    for (const loc of LOCALES) {
      map[loc] = { name: "", description: "" };
  ...
  const openEditModal = (cat: any) => {
    ...
    const map: any = {};
    for (const loc of LOCALES) {
      const found = cat.translations?.find((t: any) => t.locale === loc);
      map[loc] = {
        name: found?.name || "",
  ...
  const handleTranslationChange = (loc: Locale, field: "name" | "description", val: string) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const primaryName = translations.tr.name || translations.en.name || slug;

    const translationsPayload = LOCALES.map((loc) => ({
      locale: loc,
      name: translations[loc].name || primaryName,
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (editingCategory) {
          ...
          setSuccessMsg("Kategori başarıyla güncellendi.");
        } else {
          setCategories((prev) => [...prev, data.category]);
          setSuccessMsg("Yeni kategori başarıyla oluşturuldu.");
        }
  ...
  const handleDelete = async (cat: any) => {
    const trName = cat.translations?.find((t: any) => t.locale === "tr")?.name || cat.slug;
    if (!confirm(`"${trName}" kategorisini silmek istediğinize emin misiniz?`)) return;
  ...
  const filtered = categories.filter((c) => {
    const trName = c.translations?.find((t: any) => t.locale === "tr")?.name || "";
    const enName = c.translations?.find((t: any) => t.locale === "en")?.name || "";
    const matchSearch =
      trName.toLowerCase().includes(search.toLowerCase()) ||
      enName.toLowerCase().includes(search.toLowerCase()) ||
      c.slug.toLowerCase().includes(search.toLowerCase()) ||
      (c.icon && c.icon.toLowerCase().includes(search.toLowerCase()));
        ...
        <div className="flex flex-wrap items-center gap-2.5 flex-1">
          <div className="relative flex-1 min-w-[240px] max-w-md">
            ...
            <input
              ...
              placeholder="Kategori adı, slug veya ikon ara..."
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-border bg-surface-2/60 text-left text-foreground-muted text-[11px] uppercase tracking-wider font-semibold">
                ...
                <th className="py-3.5 px-4">Kategori Adı (TR)</th>
                <th className="py-3.5 px-4">Slug</th>
                <th className="py-3.5 px-4">İkon</th>
                <th className="py-3.5 px-4">Ürün Sayısı</th>
            ...
            <tbody className="divide-y divide-border">
              {filtered.length > 0 ? (
                filtered.map((c) => {
                  const trTrans = c.translations?.find((t: any) => t.locale === "tr");
                  const enTrans = c.translations?.find((t: any) => t.locale === "en");
                  const trName = trTrans?.name || enTrans?.name || c.translations?.[0]?.name || c.slug;
                  const locales = c.translations?.map((t: any) => t.locale) || ["tr"];
                  const pCount = c._count?.products ?? 0;

                  return (
                    <tr key={c.id} className="hover:bg-surface-2/40 transition-colors">
                      <td className="py-4 px-4 text-center font-mono font-bold text-primary">{c.sortOrder}</td>
                      <td className="py-4 px-4">
                        <div className="font-bold text-foreground">{trName}</div>
                        {trTrans?.description && (
                      ...
                      <td className="py-4 px-4 font-mono text-foreground-muted">/{c.slug}</td>
                      <td className="py-4 px-4">
                      ...
                      <td className="py-4 px-4 font-semibold text-foreground whitespace-nowrap">
                        <span className="inline-flex items-center px-2.5 py-1 rounded-full bg-primary/10 text-primary text-[10px] font-bold">
                          {pCount} Ürün
                        </span>
                      </td>
                      <td className="py-4 px-4 text-center whitespace-nowrap">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-bold ${
                          c.isActive !== false ? "bg-emerald-500/15 text-emerald-400 border border-emerald-500/30" : "bg-surface-2 text-foreground-muted border border-border"
                        }`}>
            ...
            <div className="flex items-center justify-between border-b border-border pb-4">
              <div>
                ...
                <p className="text-xs text-foreground-muted mt-0.5">
                  Kategori bilgilerini ve isteğe bağlı çok dilli çevirilerini girin.
                </p>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div className="space-y-1 sm:col-span-2 flex items-center gap-3 pt-4">
                  <label className="flex items-center gap-2 cursor-pointer text-xs font-semibold text-foreground">
                    ...
                    />
                    <span>Kategori Sitede Aktif Olsun</span>
                  </label>
                </div>
              </div>
              ...
              <div className="space-y-4 pt-4 border-t border-border">
                <div className="flex items-center justify-between">
                  <label className="text-xs font-bold text-foreground flex items-center gap-1.5">
                    ...
                    <span>Kategori İsim ve Açıklama Çevirileri (İsteğe Bağlı)</span>
                  </label>
                  <span className="text-[11px] text-foreground-muted">
                    Boş bırakılan diller ana dilden otomatik devralınır.
                  </span>
                </div>
                ...
                <div className="flex items-center gap-1.5 overflow-x-auto pb-1">
                  {LOCALES.map((loc) => {
                    ...
                    const isFilled = !!translations[loc].name;
                    return (
                      <button
                        key={loc}
                        type="button"
                ...
                <div className="p-4 rounded-2xl bg-surface-2/60 border border-border space-y-3">
                  <div className="space-y-1">
                    <label className="block text-[11px] font-bold text-foreground">
                      Kategori Adı ({LOCALE_METADATA[activeLangTab].name})
                    </label>
                    <input
                      type="text"
                      value={translations[activeLangTab].name}
                      onChange={(e) => handleTranslationChange(activeLangTab, "name", e.target.value)}
                      placeholder={`Örn: Ameliyathane Cihazları (${activeLangTab.toUpperCase()})`}
                  ...
                  <div className="space-y-1">
                    <label className="block text-[11px] font-bold text-foreground">
                      Kısa Açıklama ({LOCALE_METADATA[activeLangTab].name})
                    </label>
                    <textarea
                      rows={2}
              ...
              <div className="flex items-center justify-end gap-3 pt-4 border-t border-border">
                ...
                >
                  ...
                  <span>{editingCategory ? "Güncelle" : "Kategoriyi Kaydet"}</span>
                </button>
              </div>
            </form>
          </div>
